...
- Linux continues to check GitHub Releases for newer versions but no longer
  ...
  dialog now opens the release page so users can choose the appropriate Linux
  package; Windows keeps the existing automatic installer flow unchanged.

#### Release automation (§16)
- Codecov now enforces explicit 80% project and patch coverage targets, and
  the required lint/test job runs independently of release-info resolution so
  a skipped dependency cannot satisfy the branch-protection gate.
- **Release notes generation:** GitHub release notes are now generated by
  GitHub's native `generate_release_notes` (via `softprops/action-gh-release`)
  instead of being parsed out of `CHANGELOG.md`. This lists the merged PRs
  since the previous release directly from git/PR history, so notes are
  accurate for every release — including dependency-bump and chore releases
  that don't touch `CHANGELOG.md` — without depending on the changelog being
  kept in sync with each tag. `CHANGELOG.md` itself is unaffected and
  continues to be maintained per the release checklist; it's just no longer
  the release-notes data source. Removed `scripts/release_notes.py`, which
  implemented the old changelog-parsing approach.

### Added

#### Linux release packaging (§7.1, §12.3, §16)
- Added DEB, RPM, AppImage, Flatpak, and Snap release assets for both amd64 and
  arm64, built natively and attached to the existing per-tag GitHub release.
- DEB/RPM packages depend on system `ffmpeg` (with RPM Fusion required on
