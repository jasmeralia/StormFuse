...
| FFmpeg (BtbN/FFmpeg-Builds static builds) | GPL-3.0 | Shipped as separate Linux executables in AppImages; invoked via subprocess |
| FFmpeg (distribution packages/runtimes) | Distribution-specific GPL build | Required by DEB/RPM, supplied by the Flatpak runtime extension, or staged into Snap |
...
- `resources/licenses/FFMPEG-SOURCE.txt` — upstream URLs, versions, build dates,
  SHA-256 hashes, and source pointers for the bundled gyan.dev Windows and
  BtbN/FFmpeg-Builds Linux AppImage builds.
...
| Concern | Decision |
|---------|----------|
| Target OS | Windows 10 / 11 (64-bit); secondary Linux amd64/arm64 packages |
| Python | 3.14+ (bundled by PyInstaller) |
| UI toolkit | PyQt6 |
| Media toolchain | Windows: pinned gyan.dev build; Linux: format-specific dependency/bundle described in §12.3 |
| Installer | Windows: PyInstaller → NSIS; Linux: DEB, RPM, AppImage, Flatpak, and Snap |
| CI | GitHub Actions (`ubuntu-latest`, `ubuntu-24.04-arm`, and `windows-latest`) |

Windows remains the primary target. Linux is a secondary target with native
amd64 and arm64 packages attached to each GitHub release. Linux ffmpeg
resolution uses a bundled binary when the package format supplies one and
otherwise falls back to the package/runtime-provided executable on `PATH`.
...
1. PyInstaller bundle: `sys._MEIPASS / "resources" / "ffmpeg" / "{base}"`.
   AppImage builds populate this directory; other Linux formats do not.
...
### 12.3 Linux packaging pipeline

`make installer` remains the common Linux staging step and produces
`dist/StormFuse/StormFuse`. Release CI converts that onedir application into
five package formats for both amd64 and arm64 using native GitHub-hosted
runners. All ten files are attached to the same tag's GitHub release; none are
published to an external repository or store.

| Format | Builder | ffmpeg / ffprobe handling |
|--------|---------|---------------------------|
| `.deb` | nFPM | Declares `Depends: ffmpeg`; uses the distribution binaries on `PATH`. |
| `.rpm` | nFPM | Declares `Requires: ffmpeg`; Fedora/RHEL users must enable RPM Fusion or an equivalent codec repository. |
| AppImage | appimagetool | Bundles SHA-256-pinned BtbN/FFmpeg-Builds static `ffmpeg` and `ffprobe` builds (NVENC-capable, unlike other static-build sources) selected for the native architecture. |
| Flatpak | flatpak-builder | Uses `org.kde.Platform` plus `org.freedesktop.Platform.ffmpeg-full`; the runtime's `/usr/bin/ffmpeg` and `/usr/bin/ffprobe` remain on `PATH` while the extension supplies the full codec libraries. |
| Snap | Snapcraft (`core24`, classic confinement) | Stages Ubuntu's `ffmpeg` package into the snap with `stage-packages`. Classic (not strict) confinement, since strict's `home`/`removable-media` plugs can't reliably reach files outside `$HOME` without portal support; install requires `--classic`. |

DEB/RPM install the onedir tree under `/usr/lib/stormfuse` with a
...
- Jobs:
  ...
  5. `build-linux-deb-rpm` on native amd64/arm64 Ubuntu runners (gated on
     `is_release`): build the plain onedir output, install verified nFPM, and
     produce architecture-specific DEB and RPM packages.
  6. `build-appimage` on native amd64/arm64 Ubuntu runners (gated on
