...
| FFmpeg (John Van Sickle static builds) | GPL-3.0 | Shipped as separate Linux executables in AppImages; invoked via subprocess |
...
- `resources/licenses/FFMPEG-SOURCE.txt` — upstream URLs, versions, build dates,
  SHA-256 hashes, and source pointers for the bundled gyan.dev Windows and John
  Van Sickle Linux AppImage builds.
...
| AppImage | appimagetool | Bundles SHA-256-pinned John Van Sickle static `ffmpeg` and `ffprobe` builds selected for the native architecture. |
...
| Snap | Snapcraft (`core24`) | Stages Ubuntu's `ffmpeg` package into the snap with `stage-packages`. |
